...
QGroupingProxyModel::QGroupingProxyModel(QObject *parent) : QAbstractItemModel(parent), d(new QGroupingProxyModelPrivate(this))
{    
	d->modelColumn = 1;
...
QGroupingProxyModelGroup* QGroupingProxyModel::addGroup(const QString & text, const QVariant & value, QGroupingProxyModelGroup* parent)
{
	if(parent == nullptr) parent = d->root;
	QGroupingProxyModelGroup* g = new QGroupingProxyModelGroup(parent);
...
QGroupingProxyModelGroup* QGroupingProxyModel::addGroup(const QIcon & icon, const QString &text, const QVariant &value, QGroupingProxyModelGroup* parent)
{
	QGroupingProxyModelGroup* g = this->addGroup(text, value, parent);
...
QModelIndex QGroupingProxyModel::index(int row, int column, const QModelIndex & parent) const
{
	...
	mParentItem = (!parent.isValid() ? d->root : static_cast<QGroupingProxyModelGroup*>(parent.internalPointer()));
...
{
	auto find = [&text](QGroupingProxyModelGroup* parent)
	{
		for (int i = 0; i < parent->childCount(); i++)
		{
			if (parent->child(i)->data(Qt::DisplayRole).toString() == text)
			{
				return i;
			}
		}
...
int QGroupingProxyModel::rowCount(const QModelIndex & parent) const
{
	...
	parentItem = (!parent.isValid() ? d->root : static_cast<QGroupingProxyModelGroup*>(parent.internalPointer()));
